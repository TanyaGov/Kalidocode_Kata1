...
namespace Kalidocode_Kata1
{
    public class Calculator
    {
        ...
        public int CustomDelimiters(string input)
        {
            ...
            if (input.Contains("["))
            {
                int index = input.LastIndexOf(']'); string stringDelimiters = input.Substring(2, index + 1);
                string[] arrDelimiters = stringDelimiters.Split(new string[] { "[", "]" }, StringSplitOptions.RemoveEmptyEntries);
                string[] arrNumbers = stringNumbers.Split(arrDelimiters, StringSplitOptions.RemoveEmptyEntries);
                foreach (string number in arrNumbers)
                {
